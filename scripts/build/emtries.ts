...
export function getEntries(project: Project): Entry[] {
  const result = readdirSync(join(project.dir, 'src'), {
    withFileTypes: true,
    encoding: 'utf-8'
  })
    .filter(dirent => !excludeDirRegex.test(dirent.name) && !excludeFileRegex.test(dirent.name))
    .map(dirent => {
      if (dirent.isFile()) {
        const rawFile = removeExt(dirent.name);
        return [rawFile === 'index' ? '.' : `./${rawFile}`, `./${join('.', 'src', dirent.name)}`];
      }
